...
if len(sys.argv) < 4:
    print("Usage: python3 script.py <total_time> <interval> <filename>")
    sys.exit(1)
...
csvwriter.writerow(["Elapsed_Time_s", "CPS", "Total_Counts", "Total_Intervals"])
...
with CapeMCA() as mca:
	try:
		...
		while time.time() - start < duration:
			...
			csvwriter.writerow([elapsed, status.cps, status.total_count, status.total_intervals])
